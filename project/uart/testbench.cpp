	...
	for(t=0;t<test.size();t++){
		...
		if(!Verilated::gotFinish()){
	    	...
	    	for(j=0;j<INPUT_PARTS;j++){
	    		...
	    		// printf ("[%d] %d,   ",j,data);
	    		switch(j){
	    			case 0: top->ld_tx_data = data; break;
	    			case 1: top->tx_data = data; break;
	    			case 2: top->tx_enable = data; break;
	    			case 3: top->uld_rx_data = data; break;
	    			case 4: top->rx_enable = data; break;
	    			case 5: top->rx_in = data; break;
	    		}
	    	}
	    	...
	    	if(PRINT_OUTPUT){
	    		for(j=0;j<OUTPUT_PARTS;j++){
		    		switch(j){
		    			case 0: printf("%X_",top->tx_out); break;
		    			case 1: printf("%X_",top->tx_empty); break;
		    			case 2: printf("%X_",top->rx_data); break;
		    			case 3: printf("%X",top->rx_empty); break;
		    		}
	    		}
	    		cout<<endl;
	    	}

	...
	for(t=0;t<test.size();t++){
		...
		if(!Verilated::gotFinish()){
	    	...
	    	for(j=0;j<INPUT_PARTS;j++){
	    		...
	    		// printf ("[%d] %d,   ",j,data);
	    		switch(j){
	    			case 0: top->r_button = data; break;
	    			case 1: top->g_button = data; break;
	    			case 2: top->key = data; break;
	    			case 3: top->start = data; break;
	    			case 4: top->test = data; break;
	    			case 5: top->rts = data; break;
	    			case 6: top->rtr = data; break;
	    			case 7: top->v_in = data; break;
	    		}
	    	}
	    	...
	    	if(PRINT_OUTPUT){
	    		for(j=0;j<OUTPUT_PARTS;j++){
		    		switch(j){
		    			case 0: printf("%X",top->cts); break;
		    			case 1: printf("%X",top->ctr); break;
		    			case 2: printf("%X",top->v_out); break;
		    		}
	    		}
	    		cout<<endl;
	    	}
